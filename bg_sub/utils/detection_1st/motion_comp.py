...
class MotionComp:
    def __init__(self,frame) -> None:
        ...
        self.win_size = params.GRID_SIZE
    ...
    def divide_image(self,frame):
      grids = np.zeros((self.vertical_grid,self.horizontal_grid))
      for i in self.vertical_grid:
        for j in self.

    def find_corners(self, frame, debug=False):

      # corners for image at time t
      p0 = cv2.goodFeaturesToTrack(frame,maxCorners=100,qualityLevel=0.3,minDistance=7)

      # perform KLT with the image from time t-1
      
      p1, st, err = cv2.calcOpticalFlowPyrLK(frame, self.prev_frame, p0, None, **self.lk_params)

      # good points to compute the homography
      good_t = p0[st==1]
      good_t_1 = p1[st==1]

    def make_homography(self,good_t,good_t_1):
      h, status = cv2.findHomography(good_t,good_t_1,method=cv2.RANSAC)
